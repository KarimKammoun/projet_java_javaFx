...
public class MemberBooksController {
    ...
    private void loadAvailableBooks() {
        ...
        try (var conn = DatabaseUtil.connect();
             var pst = conn.prepareStatement("SELECT isbn, title, author, category, available_copies FROM books WHERE available_copies > 0 AND admin_id = ? ORDER BY title")) {
    ...
    private void goBack() {
        ...
        SceneManager.loadScene("/fxml/user/member_home.fxml");
    }
