...
public class MemberHomeController {
    ...
    private void loadLibraryStats() {
        try (var conn = DatabaseUtil.connect();
            ...
            try (var rs = stmt.executeQuery("SELECT COUNT(DISTINCT isbn) as total_books, COUNT(*) as total_copies FROM copies")) {
                if (rs.next()) {
            ...
            try (var rs = stmt.executeQuery(
                    ...
                    "COUNT(CASE WHEN status='Borrowed' THEN 1 END) as borrowed FROM copies")) {
                if (rs.next()) {
    ...
    private void loadFeaturedBooks() {
        ...
        try (var conn = DatabaseUtil.connect();
             var stmt = conn.createStatement();
             var rs = stmt.executeQuery(
                 "SELECT isbn, title, author, available_copies FROM books WHERE available_copies > 0 ORDER BY title")) {
            while (rs.next()) {
    ...
    private void goToBorrowingHistory() {
        SceneManager.loadScene("/fxml/member_borrowing_history.fxml");
    }

    @FXML private void goToBrowseBooks() {
        SceneManager.loadScene("/fxml/member_books.fxml");
    }
}
