    ...
    private void saveMember() {
        ...
        try (Connection conn = DatabaseUtil.connect()) {
            ...
            if (editMode && originalPhone != null) {
                ...
                // if email changed, ensure new email not already used by another user
                if (email != null && !email.isEmpty()) {
                    try (PreparedStatement checkEmail = conn.prepareStatement("SELECT COUNT(*) FROM users WHERE email = ? AND phone <> ?")) {
                        checkEmail.setString(1, email);
                        checkEmail.setString(2, originalPhone);
            ...
            // if email provided, ensure it's not used already
            if (email != null && !email.isEmpty()) {
